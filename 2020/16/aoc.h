...
void arr_set(INT_ARRAY arr, int idx, int val) {
  if (idx < 0) {
  ...
  if (idx < arr->currsize) {
    arr->data[idx] = val;
  } else if (idx < arr->maxsize) {
    arr->currsize = idx;
    arr->data[idx] = val;
  } else {
    int prevsize = arr->maxsize;
    while (arr->maxsize < idx)
      arr->maxsize *= 2;

    int *newarr = (int*)malloc(sizeof(int) * arr->maxsize);
    if (newarr == NULL) {
      perror("failed to malloc new array");
      exit(1);
    }
    memcpy(newarr, arr->data, sizeof(int) * prevsize);
    free(arr->data);
    arr->data = newarr;
    arr_set(arr, idx, val);
  }
...
void mat_set(INT_MATRIX mat, int r, int c, int val) {
  mat->data[r]->data[c] = val;
}
...
int arr_eq(INT_ARRAY a, INT_ARRAY b) {
  if (len(a) != len(b))
    return 0;
  for (int j = 0; j < len(a); j++) {
    if (arr_get(a, j) != arr_get(b, j))
      return 0;
  }
  return 1;
}
int mat_eq(INT_MATRIX a, INT_MATRIX b) {
  if (height(a) != height(b) || width(a) != width(b))
    return 0;
  for (unsigned int i = 0; i < height(a); i++) {
    if (!arr_eq(a->data[i], b->data[i]))
      return 0;
  }
  return 1;
}

int inside(INT_MATRIX mat, int i, int j) {
  if (i < 0 || i >= height(mat))
    return 0;
  if (j < 0 || j >= width(mat))
    return 0;
